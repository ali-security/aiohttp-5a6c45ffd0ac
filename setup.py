...
#
# Brotli's public headers live in ``c/include/brotli`` upstream; here they are
# vendored as ``c/inc/brotli`` because the repository ``.gitignore`` ignores any
# directory named ``include/`` (a leftover virtualenv scratch pattern), which
# would silently drop the headers from source control and break the build.
...
_brotli_extension = Extension(
    ...
    include_dirs=[f"{_BROTLI_VENDOR}/c/inc"],
)
