...
# brotlicffi compiles against the same vendored Brotli C tree the CPython
# ``_brotli`` extension uses (one shared copy, not a second libbrotli/). Paths are
# resolved from the build CWD (the repo root) by setuptools/cffi.
_VENDOR = "aiohttp/_vendored/brotli_src"

ffi.set_source(
    "aiohttp._vendored.brotlicffi._brotlicffi",
    """#include <brotli/decode.h>
       #include <brotli/encode.h>
    """,
    libraries=libraries,
    include_dirs=[f"{_VENDOR}/c", f"{_VENDOR}/c/inc", f"{_VENDOR}/c/common"],
)
