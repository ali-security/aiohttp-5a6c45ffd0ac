...
async def test_load_does_not_execute_payload_side_effect(tmp_path: Path) -> None:
    """Verify the payload's side effect never happens while loading.

    The payload below is a raw pickle stream (``GLOBAL os mkdir`` followed by
    ``REDUCE``): plain ``pickle.load()`` would call ``os.mkdir()`` and create
    ``marker``. The load must be refused before the callable is ever invoked.
    """
...
async def test_load_rejects_protocol_0_global_payload(tmp_path: Path) -> None:
    """Verify the legacy GLOBAL opcode is restricted as well.

    Protocol 0/1 pickles resolve globals through a different opcode than
    protocol 4+, so the restriction must cover that path too.
    """
    import os
...
async def test_save_writes_json_not_pickle(
    tmp_path: Path,
    cookies_to_receive: SimpleCookie,
) -> None:
    """Verify save() no longer emits a pickle stream."""
    import json
    ...
    assert not raw.startswith(b"\x80")  # pickle PROTO opcode
    data = json.loads(raw.decode("utf-8"))
    assert any("domain-cookie" in cookies for cookies in data.values())
...
async def test_save_load_json_partitioned_cookies(tmp_path: Path) -> None:
    ...
    # Compare individual cookie values
    saved = list(jar_save)
...
async def test_load_json_replaces_existing_jar_contents(tmp_path: Path) -> None:
    """Verify load() replaces the jar rather than merging onto prior state."""
    file_path = tmp_path / "replace.json"
    url = URL("https://example.com/")

    jar_save = CookieJar()
    jar_save.update_cookies_from_headers(
        ["fresh=new; Domain=example.com"], URL("https://example.com/")
    )
    jar_save.save(file_path=file_path)

    jar_load = CookieJar()
    jar_load.update_cookies_from_headers(
        ["stale=old; Domain=example.com"], URL("https://example.com/")
    )
    jar_load.load(file_path=file_path)

    filtered = jar_load.filter_cookies(url)
    assert "fresh" in filtered
    assert "stale" not in filtered
    assert len(jar_load) == 1


async def test_load_json_applies_acceptance_rules(tmp_path: Path) -> None:
    """Verify a persisted IP-host cookie is rejected by a jar without unsafe=True."""
    file_path = tmp_path / "ip.json"
    url = URL("http://127.0.0.1/")

    jar_save = CookieJar(unsafe=True)
    jar_save.update_cookies({"sid": "x"}, response_url=url)
    assert "sid" in jar_save.filter_cookies(url)
    jar_save.save(file_path=file_path)
