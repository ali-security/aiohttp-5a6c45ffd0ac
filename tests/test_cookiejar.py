...
async def test_load_json_applies_update_cookies_acceptance_rules(
    tmp_path: Path,
) -> None:
    """Verify a cookie for an IP-address host is refused by a safe jar on load."""
    file_path = tmp_path / "ip.json"
    url = URL("http://127.0.0.1/")

    jar_save = CookieJar(unsafe=True)
    jar_save.update_cookies({"sid": "ipsecret"}, response_url=url)
    jar_save.save(file_path=file_path)

    # A jar created without unsafe=True does not accept cookies from IPs.
    jar_load = CookieJar()
    jar_load.load(file_path=file_path)
    assert len(jar_load) == 0

    # An unsafe jar still accepts it, host-only scope included.
    jar_unsafe = CookieJar(unsafe=True)
    jar_unsafe.load(file_path=file_path)
    assert jar_unsafe._host_only_cookies == {("127.0.0.1", "sid")}
    assert "sid" in jar_unsafe.filter_cookies(url)
