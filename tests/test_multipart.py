...
from aiohttp.compression_utils import DEFAULT_MAX_DECOMPRESS_SIZE, ZLibBackend
...
class TestPartReader:
    ...
    def test_decode_with_content_encoding_size_limit(self) -> None:
        """Body part decompression is capped (CVE-2025-69223).

        A tiny compressed part that expands to 4MiB must not be inflated past
        ``max_decompress_size``.
        """
        original = b"A" * (4 * 2**20)
        compressor = zlib.compressobj(wbits=-zlib.MAX_WBITS)
        compressed = compressor.compress(original) + compressor.flush()
        assert len(compressed) < 2**14

        with Stream(b"%s--:--" % newline) as stream:
            obj = aiohttp.BodyPartReader(
                BOUNDARY,
                {CONTENT_ENCODING: "deflate"},
                stream,
                max_decompress_size=1024,
            )
            result = obj.decode(compressed)

        assert len(result) <= 1024

    def test_decode_with_content_encoding_default_size_limit(self) -> None:
        """The default body part decompression cap is applied.

        The compressed payload is built incrementally so the bomb is never
        materialised by the test itself.
        """
        compressor = zlib.compressobj(wbits=-zlib.MAX_WBITS)
        chunks = [compressor.compress(b"A" * 2**20) for _ in range(64)]
        ...
        assert len(result) <= DEFAULT_MAX_DECOMPRESS_SIZE
        assert len(result) < 64 * 2**20
